...
function M.get_pending_prs()
    ...
    local pr_data =
        vim.fn.system 'gh search prs --review-requested @me --limit 10 --json title,url,repository 2>/dev/null'
...
function M.get_my_open_prs()
    ...
    local pr_data =
        vim.fn.system 'gh search prs --author @me --state open --limit 10 --json title,url,repository 2>/dev/null'
    if pr_data == '' or vim.v.shell_error ~= 0 then
...
function M.get_yesterdays_commits()
    ...
    local gh_cmd =
        string.format("gh search commits --author=@me --committer-date='%s' --limit 20 2>/dev/null", yesterday)
    local commits_data = vim.fn.system(gh_cmd)

    if not commits_data or commits_data == '' or vim.v.shell_error ~= 0 then
        return nil,
            string.format('No commits found for yesterday (%s) or not authenticated with GitHub CLI.', yesterday)
    end
